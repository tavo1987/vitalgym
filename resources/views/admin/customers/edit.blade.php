...
@section('contentheader_title')
    Editando cliente - {{ $customer->full_name }}
@endsection
...
@section('main-content')
    <div class="box">
        <div class="box-header tw-mb-3 tw-py-0 tw-mb-2">
            <h2 class="tw-text-base">Editar los campos que desea del cliente</h2>
        </div>
        <div class="box-body">
            <form class="tw-rounded-lg tw-shadow tw-bg-grey-lightest tw-px-10 tw-py-6" action="{{ route('admin.customers.update', $customer) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('patch')
                ...
                <div class="form-group {{ $errors->has('name') ? ' has-error': '' }}">
                    <img class="tw-w-48 tw-h-48 tw-rounded-full tw-mb-4" src="{{ Storage::url($customer->avatar) }}" alt="Avatar">
                ...
                <div class="xl:tw-flex xl:tw-flex-wrap">
                    ...
                    <div class="form-group xl:tw-w-1/2 xl:tw-pl-2 {{ $errors->has('last_name') ? ' has-error': '' }}">
                        ...
                        <input class="form-control" type="text" name="last_name" value="{{ old('last_name', $customer->last_name) }}">
                    ...
                    <div class="form-group xl:tw-w-1/2 xl:tw-pr-2 {{ $errors->has('ci') ? ' has-error': '' }}">
                        ...
                        <input class="form-control" type="number" name="ci" value="{{ old('ci', $customer->ci) }}">
                        @if ($errors->has('ci'))
                    ...
                    <div class="form-group xl:tw-w-1/2 xl:tw-pl-2 {{ $errors->has('cell_phone') ? ' has-error': '' }}">
                        <label>Celular:</label>
                        <input class="form-control" type="tel" name="cell_phone" value="{{ old('cell_phone', $customer->cell_phone) }}">
                ...
                <div class="form-group {{ $errors->has('birthdate') ? 'has-error': '' }}">
                    ...
                    <input class="form-control datepicker" type="text" name="birthdate" value="{{ old('birthdate', $customer->birthdate->format('Y-m-d')) }}">
                ...
                <div class="xl:tw-flex">
                    <div class="form-group xl:tw-w-1/3 xl:tw-pr-4 {{ $errors->has('gender') ? 'has-error': '' }}">
                        ...
                        <select name="gender" class="form-control">
                            ...
                            <option value="feminine" {{ old('gender',$customer->gender) == 'feminine' ? 'selected' : '' }}>Femenino</option>
                        </select>
                    ...
                    <div class="form-group xl:tw-w-1/3 {{ $errors->has('routine_id') ? 'has-error': '' }}">
                        ...
                        <select name="routine_id" class="form-control">
                            ...
                            @foreach($routines as $routine)
                                <option value="{{ $routine->id }}" {{ old('routine_id', $customer->routine_id) == $routine->id ? 'selected' : '' }}>
                                    {{ $routine->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group {{ $errors->has('medical_observations') ? 'has-error': '' }}">
                    ...
                    <textarea class="form-control" type="text" name="medical_observations">{{ old('medical_observations', $customer->medical_observations) }}</textarea>
